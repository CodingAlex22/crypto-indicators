...
function getCharts() {
    const getData = async () => {
        const url = `https://vercel-proxy-1-rb0vg4434-codingalex22s-projects.vercel.app/${localStorage.getItem('userinput')}USDT/${localStorage.getItem('userselect')}`
    ...
    //RSI
     ...
     //RSI
     const stochrsi_series = chart.addLineSeries({
        color: 'lightblue',
        lineWidth: 1,
        pane: 1,
      });
      const stochrsi_data = klinedata
        .filter((d) => d.stochrsi)
      ...
      candleseries.setMarkers(
        klinedata.filter((d) => d.longStochRSI || d.shortStochRSI)
          .map((d) => 
            d.longStochRSI
              ? {
                time: d.time,
